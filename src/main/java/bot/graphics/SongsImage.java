...
public class SongsImage extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		for (int i = 0; i < scores.size(); i++) {
			...
			boolean isRanked = stars > 0;
			int rankOnPlayerLeaderboard = isRanked ? Format.roundDouble((Math.log10(score.getWeight()) + Math.log10(0.965)) / Math.log10(0.965)) : -1;
			...
			// Accuracy
			Text accText = new Text(rightTextsX + 880, rightTextsY + 80, acc);
			accText.setFont(FontUtils.consolasBold(70));
			accText.setFill(Color.WHITE);
			accText.setEffect(textShadow);

			// Rank
			Text rankText = new Text(rightTextsX + 980, rightTextsY + 150, "#" + rank);
			rankText.setFont(FontUtils.consolasBold(60));
			rankText.setFill(rankColor);
			rankText.setEffect(textShadow);

			root.getChildren().addAll(songNameText, authorText, diffText, accText, rankText);
			...
			if (isRanked) {
				...
				System.out.println("star "+star.getX()+" "+star.getY());
				System.out.println("image "+starImage.getWidth()+" "+starImage.getHeight());
				// PP
	...
	private void saveFile(Image content, File file) {
		try {
			System.out.println("Saving SongsImage");
